...
When launching through `Start-Process`, use `-WindowStyle Normal`: `Hidden` can hide the game itself while audio continues. The tested windowed run rendered at 960 by 540. Changing display mode during an earlier run failed in FNA backbuffer recreation; keep the working display mode for this experiment.
