...
- [x] Write and run failing tests for missing assemblies, real SHA-256 output, valid raw-deflate input, corrupt input, and decompression size limits.
- [x] Implement installation inspection with stable JSON output and explicit warnings for unreadable/unsupported content.
- [x] Run module tests and commit on the assigned branch.
...
- [x] Write and run failing tests using temporary ZIPs and Pillow-created images: metadata filtering, traversal reporting, dimensions, alpha, palette limits, empty sprites, and edge contact.
- [x] Implement read-only reporting with actionable violations and no extraction or pixel mutation.
- [x] Run module tests and commit on the assigned branch.
...
- [x] Write failing CLI subprocess tests for JSON output, exit codes, and invalid inputs.
- [x] Implement argparse commands, controlled errors, packaging, and Windows/Linux CI.
- [x] Integrate reviewed agent commits and run all tests.
- [x] Run inspection against the local Steam game and ZIP; save reports to ignored artifacts/.
- [x] Obtain independent review, fix substantive findings, commit verified results.
