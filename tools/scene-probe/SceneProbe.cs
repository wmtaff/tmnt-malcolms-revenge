...
internal static class SceneProbe
{
    ...
    public static int Main(string[] args)
    {
        try
        {
            ...
            if (args.Length != 2) throw new ArgumentException("Usage: SceneProbe.exe GAME_ROOT SCENE_ZPBN (or --self-test)");
            ...
            Type foot = game.GetType("Paris.Game.Actor.FootSoldier", true);
            Console.WriteLine("Declared runtime type: " + foot.FullName);
            for (Type parent = foot.BaseType; parent != null; parent = parent.BaseType)
                Console.WriteLine("Base: " + parent.FullName);
            byte[] data = Inflate(args[1]);
            int found = 0;
            foreach (int offset in FindRecords(data, "ParisSerializer.FootSoldierReadWriter"))
            {
                using (var stream = new MemoryStream(data))
                using (var reader = new BinaryReader(stream))
                {
                    stream.Position = offset;
                    Type baseType = engine.GetType("Paris.Engine.GameObject2d", true);
                    object obj = Activator.CreateInstance(baseType);
                    object readWriter = Activator.CreateInstance(serializers.GetType("ParisSerializer.GameObject2dReadWriter", true));
            ...
            Console.WriteLine("Parsed base records: " + found);
            return found == 0 ? 1 : 0;
        }
